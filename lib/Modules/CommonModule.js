...
class CommonModule extends BaseModule_1.BaseModule {
    ...
    fetchExpandedById(id, expansions) {
        let uri = this.request[this.entityType].byId(id);
        if (expansions) {
            expansions.forEach(expansion => uri = uri.expand(expansion));
        }
        const fetchRequest = {
    ...
    delete(keyOrEntity, withFullDataErasure = false) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id, version } = yield this.resolveKeyAndVersion(keyOrEntity, this.fetchByKey.bind(this));
            let uri = this.request[this.entityType].byId(id).withVersion(version);
    ...
    deleteById(id, withFullDataErasure = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let uri = this.request[this.entityType].byId(id).withVersion(entry.version);
            if (withFullDataErasure) {
                uri = uri.withFullDataErasure();
            }
            const deleteRequest = {
